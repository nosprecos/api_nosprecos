module.exports = {
    no_username: 'Login de usuário nao informado',
    min_char_user: 'Login de usuário não atinge numero de char mínimo',
    max_char_user: 'Login de usuário passou do valor máximo',
    existing_username: 'Login de usuario ja utilizado',

    no_realname: 'Nome de usuario nao informado',
    max_char_name: 'Nome de usuário passou do valor máximo',

    no_password: 'Senha de usuario nao informado',
    mismatch_password: 'Senhas não são iguais',
    not_secured_password: 'Senha nao esta conforme nivel de seguranca',

    no_email: 'Email de usuario nao informado',
    existing_email: 'Email de usuario ja utilizado',

    cant_find_customer: 'Can not find customer'

}